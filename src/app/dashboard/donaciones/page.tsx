...
export default function DonacionesDashboardPage() {
  ...
  const handleQuickApprove = async (donationId: string) => {
    try {
      ...
      toast.error('Error al aprobar donación');
    }
  };
  ...
  const handleQuickReject = async (donationId: string) => {
    try {
      ...
      if (response.ok) {
        toast.success('Donación rechazada exitosamente');
        fetchDonations();
        fetchDonationProjects();
      } else {
        const error = await response.json();
        toast.error(error.error || 'Error al rechazar donación');
      }
    } catch (error) {
      console.error('Error al rechazar donación:', error);
      toast.error('Error al rechazar donación');
    }
  };
