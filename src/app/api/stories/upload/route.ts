...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[Stories][UPLOAD] Subiendo imagen: ${file.name} (${(file.size / (1024 * 1024)).toFixed(2)} MB)`);
    ...
    const upload = await storageService.uploadFile(buffer, file.name, {
      ...
      prefix: 'stories-images/'
    });

    console.log(`[Stories][UPLOAD] Imagen subida: ${upload.url}`);
