...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[Events][UPLOAD] Subiendo imagen: ${file.name} (${(file.size / (1024 * 1024)).toFixed(2)} MB)`);
    ...
    const uploadResult = await storageService.uploadFile(
      ...
      file.name,
      {
        ...
        prefix: 'events-images/'
      }
    );

    console.log(`[Events][UPLOAD] Imagen subida: ${uploadResult.url}`);
